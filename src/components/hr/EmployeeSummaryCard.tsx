...
import type { EmployeeSummary, Branch } from '@/types/hr'
...
export default function EmployeeSummaryCard({ summary, branches }: Props) {
  if (!summary) {
    return (
      <div className="card p-5 flex flex-col items-center justify-center text-center gap-2 min-h-[110px]">
        ...
        <p className="text-sm text-tertiary">اختر موظفاً لعرض ملخصه الشهري</p>
      </div>
    )
  }

  const { employee, currentBranchId, totalDecimalHours, remainingHours, isOverLimit, recordsCount } = summary
  const pct    = Math.min(100, (totalDecimalHours / MONTHLY_LIMIT_HOURS) * 100)
  const overBy = isOverLimit ? Math.round((totalDecimalHours - MONTHLY_LIMIT_HOURS) * 100) / 100 : 0
  ...
  return (
    <div className="card p-4 overflow-hidden" style={{ borderTop: `3px solid ${accent}` }}>
      ...
      <div className="grid grid-cols-3 gap-2 mb-3 text-center">
        {[
          { label: 'مستخدم', val: `${totalDecimalHours}س`, color: isOverLimit ? '#DC2626' : '#D97706' },
          { label: 'متبقٍ',  val: `${remainingHours}س`,   color: '#059669' },
      ...
      <div>
        <div className="flex justify-between text-[11px] text-tertiary mb-1">
          <span className="num">{totalDecimalHours} / {MONTHLY_LIMIT_HOURS} ساعات</span>
