...
function fmtDur(h: number, m: number) {
  if (h === 0) return `${m} د`
  if (m === 0) return `${h} س`
  return `${h}س ${m}د`
}
...
function MobileCard({ rec, emp, branch, over, onEdit, onDelete }: {
  rec: PermissionRecord; emp: Employee | undefined; branch: Branch | undefined
  over: boolean; onEdit: (r: PermissionRecord) => void; onDelete: (id: string) => void
}) {
  return (
    <div className="p-4 border-b border-theme last:border-0">
      ...
      <div className="flex items-center gap-2 mb-2">
        <span className="text-xs text-secondary">المدة:</span>
        <span className="text-sm font-bold text-primary num">{fmtDur(rec.hours, rec.minutes)}</span>
        <span className="text-xs text-tertiary num">({rec.decimalHours} س)</span>
      </div>
      {rec.note && (
    ...
    return (
      <div className="card p-12 flex flex-col items-center gap-3 text-center">
        ...
        <p className="text-tertiary text-xs">
          {records.length === 0
            ? 'ابدأ بإضافة أول إذن من النموذج على اليسار'
        ...
        <table className="we-table w-full min-w-[720px]">
          <thead>
            <tr>
              ...
              <th className="text-right">عشري (س)</th>
            ...
            {filtered.map(rec => {
              ...
              return (
                <tr key={rec.id}>
                  ...
                  <td><span className="num font-semibold text-sm">{fmtDur(rec.hours, rec.minutes)}</span></td>
                  <td><span className="num font-black text-sm" style={{ color: WE }}>{rec.decimalHours}</span></td>
                  <td>
